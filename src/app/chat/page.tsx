...
export default function ChatInboxPage() {
  ...
  return <AppShell>
    <PageHeader eyebrow="Private conversations" title="Chat Inbox" description="Friend, match, and group conversations sync through Supabase Realtime with voice notes, photos, replies, receipts, reactions, pins, and unread counts." action={<div className="relative w-full sm:w-64"><Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="vybe-input pl-11" placeholder="Search conversations" aria-label="Search conversations" /></div>} />
