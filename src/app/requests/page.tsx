...
export default function RequestsPage() {
  ...
  return <AppShell><PageHeader eyebrow="Connections" title="Friend Requests" description="Accept, decline, or cancel private connection requests. Changes sync in real time in Supabase mode." />
    ...
    {!incoming.length && !outgoing.length && <div className="vybe-card grid min-h-72 place-items-center rounded-[30px] p-8 text-center"><div><UserCheck className="mx-auto text-blue-400" size={30} /><h2 className="mt-5 text-xl font-black">No pending requests</h2><p className="mt-2 text-sm text-slate-500">New requests and sent connections will appear here.</p><Link href="/friends" className="vybe-button mt-6 inline-flex items-center gap-2 rounded-2xl bg-blue-600 px-5 py-3 font-black"><UserPlus size={17} /> Find members</Link></div></div>}
  </AppShell>;
}
