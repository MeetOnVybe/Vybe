...
const items = [
  ...
  { icon: MapPinOff, title: "No location sharing", body: "VYBE does not collect or expose exact location, IP address, device details, or distance." },
...
export default function SafetyPage() {
  ...
  return <AppShell>
    ...
    <section className="mt-5 grid gap-5 lg:grid-cols-[1.25fr_.75fr]"><div className="vybe-card rounded-[28px] p-6"><h2 className="flex items-center gap-2 text-xl font-black"><LockKeyhole className="text-blue-400" /> Layered moderation</h2><p className="mt-3 text-sm leading-7 text-slate-400">Potentially severe messages and stories are temporarily hidden before delivery and placed in an administrator-only queue. Human reviewers can warn, suspend, ban, remove content, restore content, and review appeals. Moderation provider instructions and credentials stay server-side.</p></div><div className="rounded-[28px] border border-blue-400/15 bg-blue-500/[.055] p-6"><AlertTriangle className="text-blue-400" /><h2 className="mt-4 text-lg font-black">Phase 4 boundaries</h2><p className="mt-2 text-sm leading-6 text-slate-500">Voice notes use microphone access only while recording. VYBE still has no public video matching, calls, live streaming, location sharing, payments, ads, reels, or anonymous public feeds.</p><p className="mt-3 text-[10px] font-black uppercase tracking-wider text-slate-600">{dataMode === "supabase" ? "Supabase production mode" : "Explicit development demo mode"}</p></div></section>
  </AppShell>;
}
